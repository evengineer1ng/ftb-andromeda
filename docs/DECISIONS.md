...
## 2026-07-04 — Avoid Creation Kit entirely; xEdit + standalone Papyrus compiler + MO2 only
Researched whether Gaming VM's toolchain could stay at "MO2, at most" rather than running Creation Kit. Findings: Papyrus compilation can be fully headless (`PapyrusCompiler.exe` ships inside Starfield's own install, not CK-specific; Caprica is a fully independent third-party compiler as a fallback). xEdit can create new records but has a confirmed, unresolved bug adding *new* aliases to a quest (corrupts past the first one) — workaround is duplicating a vanilla quest that already has the aliases needed and trimming, not adding fresh ones.

**Design change flowing from this**: Phase 0/1's checkpoint detection no longer places new trigger-volume references in the world (which would need Creation Kit's 3D viewport, or an xEdit+Blender-JSON round-trip workaround). Instead, checkpoints are `Papyrus GetDistance()` checks against pre-existing vanilla markers/statics already in the chosen location — sidesteps the placement problem rather than working around it. Toolchain on Gaming VM is now: xEdit (for the quest/global/alias work, using the duplicate-and-trim pattern) + a standalone Papyrus compiler + MO2. No Creation Kit install required.
